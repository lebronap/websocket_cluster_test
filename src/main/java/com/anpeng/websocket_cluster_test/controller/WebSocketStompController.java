...
public class WebSocketStompController {
     ...
     * restful风格，websocket中用@DestinationVariable
     *
     * @param groupId
     * @param json
    ...
    public Map<String, Object> sendMsg(@DestinationVariable(value = "groupId") String groupId,
                                       String json,
                                       StompHeaderAccessor headerAccessor) {
        //这里拿到的user对象是在WebSocketChannelInterceptor拦截器中绑定的对象
        WebSocketUserAuthentication user = (WebSocketUserAuthentication) headerAccessor.getUser();
        log.info("公告controller中获取的用户登录令牌：{}", user.getName());
        log.info("公告拿到客户端传递分组参数：{}", groupId);

        //这里拿到json字符串，其实可以自动绑定到对象上
        log.info("公告获取客户端传过来的json字符串：{}", json);
        Map msg = (Map) JSONObject.parse(json);
     ...
     * 这时，spring websocket默认会把消息推送到同一个帐号不同的session，
    ...
    public Map<String, Object> sendMsgByOwn(String json, StompHeaderAccessor headerAccessor) {
            ...
            StompHeaderAccessor headerAccessor)
    {
        ...
        simpMessagingTemplate.convertAndSendToUser(accountId, address, msg.get("msg"));
     ...
     * 3. 这里接收人的参数是用restful风格带过来了，websocket把参数带到后台的方式很多，除了url路径，
     *    还可以在header中封装用@Header或者@Headers去取等多种方式
     * @param mobile 消息接收人手机号
     * @param json 消息JSON字符串
     * @param headerAccessor
     * @return
     */
    @MessageMapping("/sendChatMsgByMobile/{mobile}")
    public void sendChatMsgByMobile(
            @DestinationVariable(value = "mobile") String mobile, String json,
            StompHeaderAccessor headerAccessor)
    {
        Map msg = (Map)JSONObject.parse(json);
        // 这里拿到的user对象是在WebSocketChannelInterceptor拦截器中绑定上的对象
        ...
        // 可以使用SimpleMessageTemplate的convertAndSendToUser方法(第一个参数是用户的登陆名username)
        String address = "/userTest/callBack";
        simpMessagingTemplate.convertAndSendToUser(mobile, address, msg.get("msg"));
